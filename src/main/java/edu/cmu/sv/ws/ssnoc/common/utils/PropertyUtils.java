...
public class PropertyUtils {
	...
	static {
		...
		try (InputStream input = SSNoCAppInitializer.class.getClassLoader()
			...
			if (input != null) {
				...
				if(SystemStateUtils.inMemoryDatabaseMode){
				    Log.warn("Initializing in-memory database ...");
				    DB_CONN_URL = "jdbc:h2:mem:test;MODE=Oracle;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE";
			    }
				else{
	...
	private static void initializeWithDefaultValuesIfNeeded() {
        if(SystemStateUtils.inMemoryDatabaseMode){
            Log.warn("Initializing in-memory database ...");
            DB_CONN_URL = "jdbc:h2:mem:";
        }
        else if (DB_CONN_URL == null) {
